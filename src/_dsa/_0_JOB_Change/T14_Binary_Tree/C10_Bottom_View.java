...
public class C10_Bottom_View {
    // Ques: https://practice.geeksforgeeks.org/problems/bottom-view-of-binary-tree/1
    ...
    TreeNode root = new TreeNode(1);
        ...
        List<List<Integer>> result = bottomView(root);
...
//            System.out.print(ans.get(i).get(0).value + ", ");
    ...
    public static List<List<Integer>> bottomView(TreeNode root) {
        ...
        bottom_view_tree_map(root, 0, 0, map);
    ...
    public static void bottom_view_tree_map(TreeNode root, int level, int position, Map<Integer, Pair> map){
        if(root == null)
            return;
        ...
        bottom_view_tree_map(root.left, level+1, position-1, map);
        bottom_view_tree_map(root.right, level+1, position+1, map);
    }
